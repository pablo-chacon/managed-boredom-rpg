...
function pickFallback(month: number): string {
  return FALLBACKS[month % FALLBACKS.length];
}

export interface NarratorInput {
  month: number;
  monthLog: string[]; // the log entries from this month
  energy: number;
  cash: number;
  employed: boolean;
  onWelfare: boolean;
...
export async function monthlyNarration(input: NarratorInput): Promise<string> {
  ...
  try {
    ...
    if (!res.ok) return pickFallback(input.month);
